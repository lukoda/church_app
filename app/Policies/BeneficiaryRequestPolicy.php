<?php

namespace App\Policies;

use Illuminate\Auth\Access\Response;
use App\Models\BeneficiaryRequest;
use App\Models\User;

class BeneficiaryRequestPolicy
{
    /**
     * Determine whether the user can view any models.
     */
    public function viewAny(User $user): bool
    {
        return $user->checkPermissionTo('view-any BeneficiaryRequest');
    }

    /**
     * Determine whether the user can view the model.
     */
    public function view(User $user, BeneficiaryRequest $beneficiaryrequest): bool
    {
        return $user->checkPermissionTo('view BeneficiaryRequest');
    }

    /**
     * Determine whether the user can create models.
     */
    public function create(User $user): bool
    {
        return $user->checkPermissionTo('create BeneficiaryRequest');
    }

    /**
     * Determine whether the user can update the model.
     */
    public function update(User $user, BeneficiaryRequest $beneficiaryrequest): bool
    {
        return $user->checkPermissionTo('update BeneficiaryRequest');
    }

    /**
     * Determine whether the user can delete the model.
     */
    public function delete(User $user, BeneficiaryRequest $beneficiaryrequest): bool
    {
        return $user->checkPermissionTo('delete BeneficiaryRequest');
    }

    /**
     * Determine whether the user can restore the model.
     */
    public function restore(User $user, BeneficiaryRequest $beneficiaryrequest): bool
    {
        return $user->checkPermissionTo('restore BeneficiaryRequest');
    }

    /**
     * Determine whether the user can permanently delete the model.
     */
    public function forceDelete(User $user, BeneficiaryRequest $beneficiaryrequest): bool
    {
        return $user->checkPermissionTo('force-delete BeneficiaryRequest');
    }
}
