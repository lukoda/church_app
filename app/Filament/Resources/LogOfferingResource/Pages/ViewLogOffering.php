...
class ViewLogOffering extends ViewRecord
{
    protected static string $resource = LogOfferingResource::class;

    protected function authorizeAccess(): void
    {
        if(auth()->user()->checkPermissionTo('view LogOffering')){
            ...
            Actions\Action::make('add_pledges')
                ...
                ->action(function(array $data){
                    foreach($data['auctioned_items'] as $item){
                        ...
                        if($item['amount_payed'] > 0){
                            $auction_item_payments = new AuctionItempayment;
                            $auction_item_payments->payment_mode = 'Cash';
                            $auction_item_payments->amount_payed = $item['amount_payed'];
                            $auction_item_payments->date_registered = now();
                            $auction_item_payments->description = 'Payment Received At Church';
                            $auction_item_payments->church_id = auth()->user()->church_id;
                            $auction_item_payments->auction_item_id = $auction_item->id;
                            $auction_item_payments->registered_by = auth()->user()->id;
                            $auction_item_payments->verification_status = 'Verified';
                        ...
                        }else if($auction_item->status == 'Auction Complete'){
                            $this->record->church_auction->update([
                ...
                Actions\Action::make('add_cash')
                    ...
                    ->action(function(array $data){
                        foreach($data['auctioned_item_payments'] as $payment){
                            ...
                            $item_payment->verification_status = 'Verified';
                            $item_payment->save();
