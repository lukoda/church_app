...
use App\Filament\Resources\ChurchDistrictResource\Pages;
use App\Filament\Resources\ChurchDistrictResource\RelationManagers;
...
class ChurchDistrictResource extends Resource
{
    ...
    {
        return auth()->user()->checkPermissionTo('view-any ChurchDistrict');
    }
                ...
                Grid::make(2)
                    ->schema([
                        Hidden::make('diocese_id')
                            ->default(ChurchDistrict::whereId(Church::whereId(auth()->user()->church_id)->pluck('church_district_id'))->pluck('diocese_id')),
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                TextColumn::make('diocese_id')
                    ->label('Diocese Name')
                    ->formatStateUsing(function($state){
                        return Diocese::whereId($state)->pluck('name')[0];
                    }),
    ...
    {
        if(auth()->user()->hasRole('Bishop')){
            return parent::getEloquentQuery()->where('diocese_id', auth()->user()->churchDistrict->diocese->id);
        }else if(auth()->user()->hasRole(['ArchBishop', 'Dinomination Admin', 'System Admin'])){
